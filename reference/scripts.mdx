---
title: "Scripts and Utility Reference for EDA Roles"
...
build_document()
  ...
  └── document.save("docs/guia_presentacion_notebooks_cleaning_eda.docx")
```
...
<Accordion title="Configuration details">
  ...
  | Heading 2 color | `#1F4D78` (dark blue) |
  | Footer | Project name, centred |
</Accordion>
...
Performs a post-processing pass over all paragraph runs in the document to fix Spanish accent characters (`é`, `á`, `ó`, `í`, `ú`, `ñ`, `ü`) that may have been corrupted during string construction.

```python
def accent_document(document: Document) -> None:
    """Fix Spanish accent characters across all runs in the document."""
```

<Note>
  This function is called once at the end of `build_document()`, after all content has been added. It iterates over every paragraph in the document body and corrects encoding issues in each run's text.
</Note>
...
Background color: `#E8EEF5` (light blue). Cell padding: 6 pt on all sides.
...
<Accordion title="Parameters">
  ...
  <ParamField path="col_widths" type="list[float]" required>
    Two-element list specifying the width in inches of the label column and the detail column respectively (e.g., `[2.0, 4.5]`).
  </ParamField>
...
<Accordion title="Parameters">
  ...
  <ParamField path="widths" type="list[float]" required>
    Column widths in inches. Must have the same length as `headers`.
  </ParamField>
</Accordion>

Header row background: `#2E74B5` (blue) with white bold text.
...
<Accordion title="Parameters">
  ...
  <ParamField path="style" type="str">
    Word paragraph style name (default: `"Normal"`). Common values used in this script: `"Heading 1"`, `"Heading 2"`, `"Normal"`.
  </ParamField>
...
The script uses a four-colour palette consistent with the project's visual identity:

| Name | Hex | Usage |
|---|---|---|
| Blue | `#2E74B5` | Heading 1, matrix table headers, title text |
| Dark blue | `#1F4D78` | Heading 2 |
| Light blue | `#E8EEF5` | Callout box backgrounds |
| Light gray | `#F2F4F7` | Label-detail table header rows |
